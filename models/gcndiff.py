...
class GCNdiff(nn.Module):
    ...
    def forward(self, x, mask, t, cemd, visibility=None):
        # timestep embedding
        ...
        if visibility is not None:
            # visibility: (batch_size, 11)
            vis_emb = self.visibility_embedding(visibility.unsqueeze(-1))  # (batch, 11, hid_dim)
            
            # 融合可见性嵌入
            out = torch.cat([out, vis_emb], dim=-1)  # (batch, 11, hid_dim*2)
            out = self.fusion_layer(out)  # (batch, 11, hid_dim)
