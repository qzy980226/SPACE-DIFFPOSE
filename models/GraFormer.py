...
class GraAttenLayer(nn.Module):
    ...
    def forward(self, x, mask, visibility_emb=None):
        # 如果有可见性嵌入，融合到特征中
        if visibility_emb is not None:
            x = x + visibility_emb
...
class GraFormer(nn.Module):
    def __init__(self, adj, hid_dim=128, coords_dim=(2, 3), num_layers=4,
        ...
        if self.use_visibility:
            self.visibility_embedding = nn.Sequential(
                nn.Linear(1, hid_dim // 2),
                nn.ReLU(),
                nn.Linear(hid_dim // 2, hid_dim)
            )

    def forward(self, x, mask, visibility=None):
        out = self.gconv_input(x, self.adj)
        
        # 处理可见性嵌入
        vis_emb = None
        if self.use_visibility and visibility is not None:
            vis_emb = self.visibility_embedding(visibility.unsqueeze(-1))
        
        for i in range(self.n_layers):
            out = self.atten_layers[i](out, mask, vis_emb)
...
if __name__ == '__main__':
    adj = adj_mx_from_edges(num_pts=11, edges=speedplus_edges, sparse=False)
    model = GraFormer(adj=adj, hid_dim=128, n_pts=11)
    x = torch.zeros((1, 11, 2))
