...
class SpeedPlusDataset(MocapDataset):
    ...
    def _load_annotations(self, json_path):
        ...
        for idx, ann in enumerate(annotations):
            ...
            # 获取2D投影并计算可见性
            keypoints_2d, visibility = self._project_and_check_visibility(
                keypoints_3d, self._cameras['spacecraft'][0]
            )
            
            # 如果标注中有可见性信息，使用标注的；否则使用计算的
            if 'keypoints_visibility' in ann:
                visibility = np.array(ann['keypoints_visibility'])
            
            data['spacecraft'][image_name] = {
                'positions': keypoints_3d[np.newaxis, :, :],
                'positions_2d': keypoints_2d[np.newaxis, :, :],
                'visibility': visibility[np.newaxis, :],  # (1, 11)
    ...
    def _project_and_check_visibility(self, keypoints_3d, camera):
        """投影3D点到2D并检查可见性"""
        import cv2
        ...
        # 检查可见性（在图像边界内且在相机前方）
        visibility = np.ones(11, dtype=np.float32)
        
        # 检查是否在图像边界内
        in_bounds = (points_2d[:, 0] >= 0) & (points_2d[:, 0] < camera['res_w']) & \
                   (points_2d[:, 1] >= 0) & (points_2d[:, 1] < camera['res_h'])
        
        # 检查是否在相机前方
        in_front = keypoints_3d[:, 2] > 0
        
        visibility[~(in_bounds & in_front)] = 0.0
