...
class Diffpose(object):
    ...
    def train(self):
        ...
        if config.data.dataset == "speedplus":
            ...
            # 训练时扩大方差10倍以增加鲁棒性
            logging.info("Training with 10x global variance scaling")

            train_generator = PoseGenerator_gmm_speedplus(
                poses_train, poses_train_2d, camerapara_train, visibility_train,
                augment_uncertainty=False,      # 禁用随机增强
                global_variance_scale=10.0      # 对所有样本应用10倍方差缩放
            )
        ...
        elif config.data.dataset == "speedplus_v2":
        ...
        training_history = {
            "config": {
                ...
                "lr_decay": decay,
                "lr_gamma": gamma,
                "diffusion_timesteps": config.diffusion.num_diffusion_timesteps,
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, batch_data in enumerate(data_loader):
                ...
                camera_para = camera_para.to(self.device)
                ...
                # 计算损失，应用可见性掩码
                loss_diff = ((e - output_noise) * visibility_mask).square().sum(dim=(1, 2))
                # 归一化：除以可见关键点数量
                n_visible = visibility_mask.sum(dim=(1, 2), keepdim=True).clamp(min=1)
            ...
            if epoch % 1 == 0:
                ...
                # 检查配置中是否有自定义验证数据路径
                custom_val_json = getattr(config.data, 'val_json_path', None)
                custom_val_npz_dir = getattr(config.data, 'val_gmm_data_path', None)

                # 测试并获取结果
                p1, p2 = self.test_hyber(
                    is_train=True,
                    custom_val_json=custom_val_json,
                    custom_val_npz_dir=custom_val_npz_dir
                )

                # 记录当前epoch的训练历史
                epoch_record = {
                    "epoch": epoch + 1,
                    "train_loss": round(epoch_loss_diff.avg, 6),
                    "val_mpjpe_mm": round(p1, 2),
                    "val_p_mpjpe_mm": round(p2, 2),
                ...
                training_history["best_mpjpe_mm"] = round(best_p1, 2)

                # 保存训练历史到JSON文件
                history_path = os.path.join(self.args.log_path, "training_history.json")
    ...
    def test_hyber(self, is_train=False, custom_val_json=None, custom_val_npz_dir=None):
        cudnn.benchmark = True
        ...
        # 验证/测试时使用原始方差（不缩放）
        logging.info("Validation/Testing: Using original variance (no scaling)")

        # 检查是否使用自定义验证数据
        if custom_val_json is not None and custom_val_npz_dir is not None:
            logging.info(f"Using custom validation data:")
            logging.info(f"  JSON: {custom_val_json}")
            logging.info(f"  NPZ directory: {custom_val_npz_dir}")

            # 加载自定义验证数据
            from common.speedplus_dataset_v2 import SpeedPlusV2Dataset
            from common.data_utils_v2 import create_2d_data_speedplus_v2, fetch_speedplus_v2

            custom_dataset = SpeedPlusV2Dataset(
                train_json_path=custom_val_json,
                kpts_mat_path=config.data.kpts_mat_path,
                gmm_data_path=custom_val_npz_dir
            )

            # 创建自定义验证数据的keypoints
            keypoints_custom = create_2d_data_speedplus_v2(custom_dataset)

            poses_valid, poses_valid_2d, camerapara_valid, visibility_valid = fetch_speedplus_v2(
                ['spacecraft'], custom_dataset, keypoints_custom, stride
            )

            test_generator = PoseGenerator_gmm_speedplus(
                poses_valid, poses_valid_2d, camerapara_valid, visibility_valid,
                augment_uncertainty=False,      # 禁用随机增强
                global_variance_scale=1.0       # 使用原始方差（不缩放）
            )
        ...
        elif config.data.dataset == "speedplus":
        ...
        for i, batch_data in enumerate(data_loader):
            ...
            if visibility.sum() > 0:  # 确保至少有一个可见点
                ...
                # 计算MPJPE（使用可见性掩码）
                mpjpe_error = mpjpe(output_xyz, targets_3d, visibility_mask_joints).item() * 1000.0
