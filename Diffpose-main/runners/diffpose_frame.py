...
class Diffpose(object):
    ...
    def prepare_data(self):
        ...
        if config.data.dataset == "speedplus":
            ...
            self.keypoints_test = self.keypoints_train  #待定，需要划分训练集和测试集
    ...
    def train(self):
        ...
        # create dataloader
        if config.data.dataset == "speedplus":
            poses_train, poses_train_2d, camerapara_train = fetch_speedplus(
                self.subjects_train, self.dataset, self.keypoints_train, stride
            )
            data_loader = train_loader = data.DataLoader(
                PoseGenerator_gmm_speedplus(poses_train, poses_train_2d, camerapara_train),
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, batch_data in enumerate(data_loader):
                ...
                # to cuda
                targets_uvxyz, targets_noise_scale, _, targets_3d, camera_para = batch_data
                ...
                loss_diff = (e - output_noise).square().sum(dim=(1, 2)).mean(dim=0) #MSE损失
            ...
            if epoch % 1 == 0:
                ...
                p1, p2 = self.test_hyber(is_train=True) #MPJPE与PA-MPJPE误差 验证集上验证
    ...
    def test_hyber(self, is_train=False):
        ...
        if config.data.dataset == "speedplus":
            ...
            poses_valid, poses_valid_2d, camerapara_valid = fetch_speedplus(
                self.subjects_test, self.dataset, self.keypoints_test, stride
            )

            data_loader = valid_loader = data.DataLoader(
                PoseGenerator_gmm_speedplus(poses_valid, poses_valid_2d, camerapara_valid),
        ...
        action_error_sum = define_error_list(dataset_type="speedplus")

        for i, batch_data in enumerate(data_loader):
            data_time += time.time() - data_start

            _, input_noise_scale, input_2d, targets_3d, camera_para = batch_data
            input_action = None
            ...
            None, None, dataset_type=config.data.dataset)
