...
def parse_args_and_config():
    ...
    parser.add_argument('--actions', default='*', type=str, metavar='LIST',
                        help='actions to train/test on, separated by comma, or * for all (Human3.6M only)')
    ...
    parser.add_argument('--train_split', default=0.8, type=float,
                        help='train/test split ratio for SPEED+ dataset')
...
def main():
    ...
    try:
        runner = Diffpose(args, config) #设置diffpose参数
        runner.create_diffusion_model(args.model_diff_path) #创建扩散模型
        runner.create_pose_model(args.model_pose_path) #创建ST模型
        runner.prepare_data() #准备数据
        if args.train:
            runner.train() #训练
        else:
            _, _ = runner.test_hyber() #测试
    except Exception:
